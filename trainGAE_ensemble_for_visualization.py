...
def get_new_archs_and_add_to_dataset(dataset_name, datasets, top_k, repeat,
    ...
    found_arch_each_round.append(found_arch_list_set)
    acc_list = acc_list[: top_k]
    top_acc_list.extend([i['valid-accuracy'] for i in acc_list])
    top_test_acc_list.extend([i['test-accuracy'] for i in acc_list])
    found_arch_list_set = found_arch_list_set[: top_k]
        ...
        while len(found_arch_list_set) < sample_amount and retry < max_retry:
            ...
            if dataset_name == 'nb101':
                ...
                for arch in found_arch_list:
                    ...
                    if spec_hash not in visited_arch:
                        found_in_this_round.append(arch)
                        visited_arch.append(spec_hash)
            else:
                found_in_this_round = list(filter(lambda arch: graph_to_str(arch) not in visited_arch, found_arch_list))
...
def main(seed, dataset_name, train_sample_amount, valid_sample_amount, query_budget,
    ...
    logdir, logger = get_logdir_and_logger(os.path.join(f'search_visualization_{train_sample_amount}_{valid_sample_amount}_{query_budget}_top{top_k}_finetune{finetune}_rfinetune{retrain_finetune}_rank{is_rank_weight}_ensemble_{num_couples}NN_{n_couple_layer}*{n_hid_layer}*{n_hid_dim}',
                                                        dataset_name), f'trainGAE_ensemble_for_visualization_{seed}.log')
    random_seed = seed
    ...
    # Save found_arch_each_round to file
    with open(os.path.join(logdir, 'found_arch_each_round.pkl'), 'wb') as f:
        pickle.dump(found_arch_each_round, f)
    # Dump model to file
    with open(os.path.join(logdir, 'retrain_model.pkl'), 'wb') as f:
        pickle.dump(model, f)
    model.save_weights(os.path.join(logdir, f'modelGAE_weights_retrain'))

...
def parse_args():
    ...
    parser.add_argument('--valid_sample_amount', type=int, default=10, help='Number of samples to train (default: 50)')
...
class Trainer3(tf.keras.Model):
    def __init__(self, model_p, model_q, init_bound_value, dataset, bound_func):
        super(Trainer3, self).__init__()
        self.model_p = model_p
        self.model_q = model_q
        self.init_bound_value = init_bound_value
    ...
    def train_step(self, data):
        with tf.GradientTape() as tape:
            cur_bound = self.bound_func(self.model_p, self.model_q, self.dataset)
            loss = cur_bound - self.init_bound_value
            self.init_bound_value = min(cur_bound, self.init_bound_value)

        grads = tape.gradient(loss, self.model_q.trainable_weights)
        self.optimizer.apply_gradients(zip(grads, self.model_q.trainable_weights))

        self.loss_tracker['total_loss'].update_state(loss)
        return {key: value.result() for key, value in self.loss_tracker.items()}
...
def main(seed, dataset_name, train_sample_amount, valid_sample_amount, query_budget):
    logdir, logger = get_logdir_and_logger(dataset_name + '_risk_bound', f'trainGAE_two_phase_{seed}.log')
    random_seed = seed
    ...
    pretrained_model, p_model, ghost_model = prepare_model(nvp_config, latent_dim, num_layers, d_model, num_heads, dff,
    ...
    if train_phase[1]:
        ...
        datasets['train_full'] = mask_graph_dataset(datasets['train'], train_sample_amount, 1,
                                                 random_seed=random_seed)
        datasets['train_post'] = mask_graph_dataset(datasets['train'], train_sample_amount, 1,
                                                    random_seed=random_seed)
        datasets['train_pre'] = mask_graph_dataset(datasets['train'], train_sample_amount, 1,
                                                    random_seed=random_seed)
        datasets['valid_1'] = mask_graph_dataset(datasets['valid'], valid_sample_amount, 1,
                                                random_seed=random_seed)

        datasets['train_full'].filter(lambda g: not np.isnan(g.y))
        datasets['train_post'].filter(lambda g: not np.isnan(g.y))
        datasets['train_pre'].filter(lambda g: not np.isnan(g.y))
        datasets['valid_1'].filter(lambda g: not np.isnan(g.y))

        datasets['train_full'] = repeat_graph_dataset_element(datasets['train_full'], repeat_label)
        datasets['train_pre'].graphs = datasets['train_pre'].graphs[:20]
        datasets['train_post'].graphs = datasets['train_post'].graphs[20:]
        datasets['train_pre'] = repeat_graph_dataset_element(datasets['train_pre'], repeat_label)
        #datasets['train_post'] = repeat_graph_dataset_element(datasets['train_post'], repeat_label)
        datasets['valid_1'] = repeat_graph_dataset_element(datasets['valid_1'], repeat_label)
        with open(os.path.join(logdir, 'datasets.pkl'), 'wb') as f:
            pickle.dump(datasets, f)
        exit()
        # Train w^{0}_a
        datasets['train'] = datasets['train_pre']
        datasets['valid'] = datasets['valid_1']
        loader = to_loader(datasets, batch_size, train_epochs)
        callbacks = [CSVLogger(os.path.join(logdir, f"learning_curve_pre.csv"))]

        trainer = train(2, model, loader, 1, logdir, callbacks,
                        x_dim=x_dim, y_dim=y_dim, z_dim=z_dim, finetune=finetune)

        with open(os.path.join(logdir, 'model_20.pkl'), 'wb') as f:
            pickle.dump(model, f)

        w_a_0_weight = model.get_weights()
        # Train w_s
        datasets['train'] = datasets['train_full']
        loader = to_loader(datasets, batch_size, train_epochs)
        callbacks = [CSVLogger(os.path.join(logdir, f"learning_curve_full.csv")),]
        trainer = train(2, model, loader, train_epochs, logdir, callbacks,
                        x_dim=x_dim, y_dim=y_dim, z_dim=z_dim, finetune=finetune)

        with open(os.path.join(logdir, 'model_full.pkl'), 'wb') as f:
            pickle.dump(model, f)

        # Train w^{G}_a
        ghost_model.set_weights(w_a_0_weight)
        datasets['train'] = datasets['train_pre'] + datasets['valid_1']
        loader = to_loader(datasets, batch_size, train_epochs)
        callbacks = [CSVLogger(os.path.join(logdir, f"learning_curve_ghost.csv"))]
        trainer = train(2, ghost_model, loader, 500, logdir, callbacks,
                        x_dim=x_dim, y_dim=y_dim, z_dim=z_dim, finetune=finetune)

        with open(os.path.join(logdir, 'model_ghost.pkl'), 'wb') as f:
            pickle.dump(ghost_model, f)
    ...
    '''
    # Bound opt
    from calculate_kl_bound_BNN import kl_bound
    ...
    datasets['train'] = datasets['train_pre']
    loader = to_loader(datasets, 64, train_epochs)
    callbacks = []
    train(2, p_model, loader, 1, logdir, callbacks,
          x_dim=x_dim, y_dim=y_dim, z_dim=z_dim, finetune=finetune)
    p_model.save_weights(os.path.join(logdir, 'w_s_0'))

    q_model = copy.deepcopy(p_model)
    datasets['train'] = datasets['train_full']
    loader = to_loader(datasets, 64, train_epochs)
    callbacks = []
    # train w^s_a
    train(2, p_model, loader, train_epochs, logdir, callbacks,
          x_dim=x_dim, y_dim=y_dim, z_dim=z_dim, finetune=finetune)
    p_model.save_weights(os.path.join(logdir, 'w_s_a'))

    #p_model.load_weights('logs/cifar10-valid/20230516-012512/w_s_a')
    #q_model = copy.deepcopy(p_model)
    #q_model.load_weights('logs/cifar10-valid/20230516-012512/w_s_0')

    #init_bound = kl_bound(p_model, q_model, datasets['train_post'])
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.01)
    q_model.encoder.trainable = False
    q_model.decoder.trainable = False

    for epoch in range(10000):
        with tf.GradientTape() as tape:
            mse, bound = kl_bound(p_model, q_model, datasets['train_post'])
            loss = bound

        grads = tape.gradient(loss, q_model.trainable_weights)
        optimizer.apply_gradients(zip(grads, q_model.trainable_weights))
        print(f'Epoch {epoch}, mse {mse}, bound {loss}, loss {loss}')

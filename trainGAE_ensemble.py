...
from datasets.utils import train_valid_test_split_dataset, mask_graph_dataset, arch_list_to_set, graph_to_str, \
    weighted_graph_dataset_element, repeat_graph_dataset_element
...
class Trainer2(tf.keras.Model):
    ...
    def cal_reg_and_latent_loss(self, y, z, y_out, nan_mask, rank_weight=None):
        ...
        reg_loss = self.reg_loss_fn(y, pred_y)  # (batch_size, num_nvp)
        ...
        if self.is_rank_weight:
            ...
            reg_loss *= tf.expand_dims(reg_loss, axis=1)
    ...
    def cal_rev_loss(self, undirected_x_batch_train, y, z, nan_mask, noise_scale, rank_weight=None):
        ...
        if self.is_rank_weight:
            ...
            rev_loss *= tf.expand_dims(rank_weight, 1)
    ...
    def train_step(self, data):
        ...
        rank_weight = y_batch_train[:, 0] if self.is_rank_weight else None
...
def train(phase: int, model, loader, train_epochs, logdir, callbacks=None, x_dim=None, y_dim=None,
          z_dim=None, finetune=False, learning_rate=1e-3, no_valid=False, is_rank_weight=False):
    if phase == 1:
        trainer = Trainer1(model)
    elif phase == 2:
        trainer = Trainer2(model, x_dim, y_dim, z_dim, finetune, is_rank_weight=is_rank_weight)
    else:
...
def get_new_archs_and_add_to_dataset(dataset_name, datasets, top_k, repeat, top_list,
                                     arch_list_set, visited, top_acc_list, top_test_acc_list, logger):
    num_new_found = 0
...
def sample_arch_candidates(model, dataset_name, x_dim, z_dim, visited, sample_amount=200):
    ...
    while len(found_arch_list_set) < sample_amount and std_idx < max_retry:
        ...
        while len(found_arch_list_set) < sample_amount and retry < max_retry:
            _, _, _, found_arch_list = ensemble_eval_query_best(model, dataset_name, x_dim, z_dim, noise_scale=noise_std_list[std_idx],
...
def retrain(trainer, datasets, dataset_name, batch_size, train_epochs, logdir, top_list, logger, repeat, top_k=5):
    ...
    if found_arch_list_set is None:
        ...
        found_arch_list_set = sample_arch_candidates(trainer.model, dataset_name, trainer.x_dim, trainer.z_dim, visited,
                                                     sample_amount=200)
        found_arch_list_set = [] if found_arch_list_set is None else found_arch_list_set
    ...
    if num_new_found == 0:
        ...
        num_new_found, top_arch_list_set = get_new_archs_and_add_to_dataset(dataset_name, datasets, top_k, repeat,
                                                                            top_list, found_arch_list_set, visited,
                                                                            top_acc_list, top_test_acc_list, logger)

    datasets['train'] = weighted_graph_dataset_element(datasets['train_1'], repeat)
    ...
    trainer.fit(loader['train'].load(),
                #validation_data=loader['valid'].load(),
                epochs=max(1, int(train_epochs * 50 / len(datasets['train_1']))),
                callbacks=callbacks,
                steps_per_epoch=loader['train'].steps_per_epoch,
                #validation_steps=loader['valid'].steps_per_epoch
                )
...
def main(seed, dataset_name, train_sample_amount, valid_sample_amount, query_budget):
    ...
    if train_phase[1]:
        ...
        if is_rank_weight:
            datasets['train'] = weighted_graph_dataset_element(datasets['train_1'], repeat_label)
            #datasets['test'] = weighted_graph_dataset_element(datasets['test'], 1)
        else:
        ...
        trainer = train(2, model, loader, train_epochs, logdir, callbacks, is_rank_weight=is_rank_weight,
                        x_dim=x_dim, y_dim=y_dim, z_dim=z_dim, finetune=finetune, learning_rate=1e-3, no_valid=True)
        results = trainer.evaluate(loader['test'].load(), steps=loader['test'].steps_per_epoch)
        logger.info(str(dict(zip(trainer.metrics_names, results))))

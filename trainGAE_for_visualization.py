...
def main(seed, dataset_name, train_sample_amount, valid_sample_amount, query_budget, top_k, finetune, retrain_finetune, is_rank_weight, random_sample):
    logdir, logger = get_logdir_and_logger(
        os.path.join(f'search_visualization_{dataset_name}_finetune{finetune}_rfinetune{retrain_finetune}_rank{is_rank_weight}',
    ...
    retrain_model.save_weights(os.path.join(logdir, f'modelGAE_weights_retrain'))

...
def main(args):
    ...
    for epoch in range(1, int(budget) + 1):
        ...
        # training
        train_obj, train_results = train(train_data, model, criterion, optimizer, epoch, device, alpha, data_config,
                                         log_dir)
        ...
        if epoch % args.save_interval == 0:
            ...
            # validation
        if epoch % 5 == 0:
            valid_obj, valid_results = infer(val_data, model, criterion, optimizer, epoch, device, alpha, data_config,
                                             log_dir)

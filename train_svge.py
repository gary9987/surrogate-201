...
def main(args, log_dir):
    ...
    if not args.only_test_mode:
        ...
        for epoch in range(1, int(budget) + 1):
            ...
            if epoch % args.save_interval == 0:
                ...
                if args.data_search_space == 'ENAS':
                    evaluate(dataset, model, epoch, args.device, log_dir, data_config, DatasetENAS, ENAS=True)
                elif args.data_search_space == 'NB101':
                    evaluate(dataset, model, epoch, args.device, log_dir, data_config, Dataset101, NB101=True)
                elif args.data_search_space == 'NB201':
                    evaluate(dataset, model, epoch, args.device, log_dir, data_config, Dataset201, NB201=True)
                else:
                    raise TypeError("Unknow Seach Space : {:}".format(args.data_search_space))
        ...
        if args.data_search_space == 'ENAS':
            evaluate(dataset, model, epoch, args.device, log_dir, data_config, DatasetENAS, ENAS=True, pretrained=True)
        elif args.data_search_space == 'NB101':
            evaluate(dataset, model, epoch, args.device, log_dir, data_config, Dataset101, NB101=True, pretrained=True)
        elif args.data_search_space == 'NB201':
            evaluate(dataset, model, epoch, args.device, log_dir, data_config, Dataset201, NB201=True, pretrained=True)
        else:
            raise TypeError("Unknow Seach Space : {:}".format(args.data_search_space))
...
def evaluate(data, model, epoch, device, log_dir, data_config, Dataset, ENAS=False, NB101=False, NB201=False,
         pretrained=False):
    with torch.no_grad():
        model.eval()
    ...
    logger.info(
        "For epoch {} of model {} reconstruction accuracy for test graph {}, valid graphs {}, unique graphs {} and novel graphs {} ".format(
            epoch, model.__class__.__name__, rec, valid, unique, novel))
    config_dict = {
        'epoch': epoch,

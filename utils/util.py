...
def is_valid_NAS(g, nasbench, START_TYPE=1, END_TYPE=0):
    ...
    try:
        model_spec = model_spec.ModelSpec(matrix=adjacency_matrix, ops=node_list)
...
def save_latent_representations(Dataset, config, model, device, epoch, path, data_name, ENAS=False, NB101=False, NB201=False):
    ...
    if ENAS == True:
        ...
        Z_train, Y_val_train, Y_test_train, Y_time_train = extract_latent(data.train_dataloader, model, device,
                                                                          NB101=NB101)
    ...
    if ENAS == True:
        ...
        with open(latent_pkl_name, 'wb') as f:
            pickle.dump(
                (Z_train, Y_val_train, Y_test_train, Y_time_train, Z_test, Y_val_test, Y_test_test, Y_time_test), f)
        print('Saved latent representations to ' + latent_pkl_name)
        scipy.io.savemat(latent_mat_name,
                         mdict={
                             'Z_train': Z_train,

...
class GNNDecoder(nn.Module):
    ...
    def inference(self, c):
        ...
        while num_ones < batch_size:
            ...
            h_rev, node_atts_new_rev, edges_new_rev, non_ones = self.generator.inference(h_rev, c, edge_index_rev,
                                                                                         backwards=True)
        ...
        for i in range(batch_size):
            ...
            eb = torch.flip(torch.stack((torch.LongTensor([L[x.item()] for x in eb_rev[0]]),
                                         torch.LongTensor([L[x.item()] for x in eb_rev[1]]))), [0, 1])
...
class GetAcc(nn.Module):
    def __init__(self, model_config, dim_target=1):
        ...
        self.lin_layers = nn.ModuleList(
            [nn.Linear(self.gdim // (2 ** num), self.gdim // (2 ** (num + 1))) for num in range(self.num_layers - 1)])
